import cometExtAbi from '../abis/MyCometExtension';
...
function getExtAddress(network: Network): string {
  if (network === 'mainnet') {
    return '0xC9c445CAAC98B23D1b7439cD75938e753307b2e6';
  } else if (network === 'goerli') {
...
export function mainnetConfig<N extends 'mainnet'>(network: N): NetworkConfig<'mainnet'> {
  const extAddress: string = getExtAddress(network);
